...
public interface IEmailServiceClient
{
    ...
    /// <returns>An operation id for tracing the success of the task or serviceerror if fail</returns>
    Task<(string? OperationId, ServiceError? Error)> SendEmail(Sending.Email email);
